...
def build_hardened_envelope_and_payload(
    ...
    file_bytes = json.dumps(full_dict, indent=2).encode("utf-8")
    _, real_sha256, real_blob_sha = compute_payload_bytes_and_hash(file_bytes)
    ...
    full_dict["envelope"] = envelope.to_dict()
    return full_dict
...
def test_valid_directive_e2e_accepted(tmp_path):
    ...
    (watcher.inbox_dir / "h-valid-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
...
def test_tampered_payload_rejected(tmp_path):
    root = tmp_path / "directives"
    watcher = DirectiveWatcher(directives_root=root)
    data = build_hardened_envelope_and_payload(directive_id="h-tampered-001")

    # Tamper payload content after envelope hash generation
    data["payload"] = {"tampered": True}
    data["payload_object"]["payload"] = {"tampered": True}

    (watcher.inbox_dir / "h-tampered-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    acks = watcher.poll_inbox()
...
def test_wrong_sha256_rejected(tmp_path):
    ...
    (watcher.inbox_dir / "h-wrong-sha-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    acks = watcher.poll_inbox()
...
def test_wrong_blob_sha_rejected(tmp_path):
    ...
    (watcher.inbox_dir / "h-wrong-blob-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    acks = watcher.poll_inbox()
...
def test_non_reachable_commit_rejected(tmp_path, monkeypatch):
    ...
    (watcher.inbox_dir / "h-unreachable-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    acks = watcher.poll_inbox()
...
def test_local_only_commit_rejected(tmp_path, monkeypatch):
    ...
    (watcher.inbox_dir / "h-local-only-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    acks = watcher.poll_inbox()
...
def test_unsigned_commit_rejected(tmp_path, monkeypatch):
    ...
    (watcher.inbox_dir / "h-unsigned-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    acks = watcher.poll_inbox()
...
def test_invalid_signature_rejected(tmp_path, monkeypatch):
    ...
    (watcher.inbox_dir / "h-invalid-sig-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    acks = watcher.poll_inbox()
...
def test_valid_signature_unauthorized_signer_rejected(tmp_path):
    ...
    (watcher.inbox_dir / "h-untrusted-signer-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    acks = watcher.poll_inbox()
...
def test_remote_unavailable_fail_closed(tmp_path, monkeypatch):
    ...
    (watcher.inbox_dir / "h-remote-down-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    acks = watcher.poll_inbox()
...
def test_corrupted_queue_fail_closed(tmp_path, monkeypatch):
    ...
    (watcher.inbox_dir / "h-queue-corrupt-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    acks = watcher.poll_inbox()
...
def test_ack_before_durability_impossible(tmp_path):
    ...
    (watcher.inbox_dir / "h-ack-durability-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    acks = watcher.poll_inbox()
...
def test_duplicate_directive_no_repeat_mutation(tmp_path):
    ...
    (watcher.inbox_dir / "h-dup-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    watcher.poll_inbox()
...
def test_directive_id_collision_state_conflict(tmp_path):
    ...
    data1 = build_hardened_envelope_and_payload(directive_id="h-collision-001", commit_sha="c969f7a")
    (watcher.inbox_dir / "h-collision-001.json").write_text(json.dumps(data1, indent=2), encoding="utf-8")
    watcher.poll_inbox()
    ...
    (watcher.inbox_dir / "h-collision-001.json").write_text(json.dumps(data2, indent=2), encoding="utf-8")
    acks2 = watcher.poll_inbox()
...
def test_restart_reconstructs_exact_state(tmp_path):
    ...
    (watcher1.inbox_dir / "h-restart-state-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    watcher1.poll_inbox()
...
def test_waiting_human_restart_persistence(tmp_path):
    ...
    (watcher1.inbox_dir / "h-wait-human-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    watcher1.poll_inbox()
...
def test_toctou_attack_revalidation_blocks_execution(tmp_path):
    root = tmp_path / "directives"
    watcher = DirectiveWatcher(directives_root=root)

    data = build_hardened_envelope_and_payload(directive_id="h-toctou-001")
    (watcher.inbox_dir / "h-toctou-001.json").write_text(json.dumps(data, indent=2), encoding="utf-8")
    watcher.poll_inbox()
