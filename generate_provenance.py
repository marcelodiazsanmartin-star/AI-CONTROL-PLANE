"""
Provenance Evidence Generator for CONTROL-01 and CONTROL-02

Generates reports/CONTROL_01_02_PROVENANCE.json tracking exact Git commit ancestry
without self-referential ambiguity.
"""
...
def generate_provenance(code_under_test_sha: str, certification_artifact_commit_sha: str) -> dict:
    ...
    prov_file = reports_dir / "CONTROL_01_02_PROVENANCE.json"
    with open(prov_file, "w", encoding="utf-8") as f:
        json.dump(prov_data, f, indent=2)

    print(f"Provenance generated at {prov_file}")
    print(f"Ancestry Verified: {ancestry_ok} ({code_under_test_sha[:7]} -> {certification_artifact_commit_sha[:7]})")
    return prov_data


def main():
    parser = argparse.ArgumentParser(description="Generate CONTROL-01/02 Provenance Evidence")
